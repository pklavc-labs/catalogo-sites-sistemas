...
(() => {
  ...
  const defaults = { businessName: defaultName, heroText: '', primaryColor: '#4f46e5', secondaryColor: '#ffffff', accentColor: '#f59e0b', font: 'system-ui', buttonStyle: 'rounded', logo: '', heroImage: '' };
  let state = { ...defaults };
  ...
  function apply() {
    ...
    document.querySelectorAll('[data-custom="business-name"],.navbar-brand').forEach(element => { if (element.tagName !== 'IMG') element.childNodes.forEach(node => { if (node.nodeType === Node.TEXT_NODE) node.nodeValue = ` ${state.businessName}`; }); });
    if (state.heroText) { const title = document.querySelector('[data-custom="hero-text"],h1'); if (title) title.textContent = state.heroText; }
    ...
    try { localStorage.setItem(key, JSON.stringify({ ...state, logo: '', heroImage: '' })); } catch {}
  }
  ...
  function mount() {
    ...
    addMarker(document.querySelector('[data-custom="hero-text"],.masthead-heading,.masthead-subheading,h1'), 'name', 'Editar textos');
    ...
    const quote = document.createElement('button'); quote.type = 'button'; quote.className = 'catalog-quote-icon'; quote.dataset.demoQuote = ''; quote.title = 'Solicitar orçamento'; quote.textContent = 'R$'; document.body.append(quote);
  }
